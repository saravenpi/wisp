...
if [ "$has_active_session" = true ]; then
    ...
    tmux display-menu -x C -y C -T " Wisp " \
        ...
        "Start Custom Session" s "if command -v gum >/dev/null 2>&1; then tmux display-popup -x C -y C -w 50 -h 6 -E 'duration=\$(gum input --no-show-help --placeholder \"Duration in minutes\" --prompt \"Duration > \" --width 30); if [ \$? -eq 0 ] && [ -n \"\$duration\" ]; then name=\$(gum input --no-show-help --placeholder \"Session name (press Enter to skip)\" --prompt \"Session > \" --width 40); if [ \$? -eq 0 ] && [ -n \"\$name\" ]; then WISP_NOTIFICATIONS=\"\${WISP_NOTIFICATIONS:-true}\" $WISP_CMD start \"\$duration\" \"\$name\"; else WISP_NOTIFICATIONS=\"\${WISP_NOTIFICATIONS:-true}\" $WISP_CMD start \"\$duration\"; fi; fi'; else tmux command-prompt -p 'Duration (minutes):' 'command-prompt -p \"Session name (Enter to skip):\" \"run-shell \\\"WISP_NOTIFICATIONS=\\\\\\\"\${WISP_NOTIFICATIONS:-true}\\\\\\\" $WISP_CMD start %1 \\\\\\\"%1\\\\\\\"\"\"'; fi" \
        "" \
        "Show History" h "display-popup -x C -y C -w 90 -h 25 -S -E '$WISP_CMD history; echo; echo \"Press any key to close...\"; read -n 1'" \
        "Show Stats" t "display-popup -x C -y C -w 70 -h 20 -S -E '$WISP_CMD stats; echo; echo \"Press any key to close...\"; read -n 1'"
fi